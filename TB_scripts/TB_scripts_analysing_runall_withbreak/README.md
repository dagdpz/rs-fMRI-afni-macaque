...
* This run_all.tcsh script is different, because now the user can choose the analysis type and type in their choice via standard input. In the former run\_all.tcsh script the user couldn't choose and the script did both voxel-based and ROI-based analysis. In this script there is another difference, it will stop after running run\_13\_aw.tcsh to let the user correct the skull stripping manually.
